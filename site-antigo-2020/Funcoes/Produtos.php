...
class Produtos extends Conexao{
    ...
    public function listarTodosProdutos() {
        ...
        $listar = $pdo->prepare('SELECT p.*, l.linha as linha FROM `produtos` p INNER JOIN linhas l ON p.linha = l.id WHERE p.ativo= ? AND l.url =? AND (p.data_postagem <= ? OR p.data_postagem IS NULL) ORDER BY LOCATE( "c", p.titulo ) = 0, LOCATE( "c", p.titulo ) LIMIT ' . $inicio . ", " . $limite);
		$listar->bindValue(1, 1);
		$listar->bindValue(2, $linha);
        $listar->bindValue(3, date('Y-m-d H:i:s'));
        $listar->execute();
        $total = $listar->rowCount();
        
        $listarPaginacao = $pdo->prepare('SELECT p.*, l.linha as linha FROM `produtos` p INNER JOIN linhas l ON p.linha = l.id WHERE p.ativo= ? AND l.url =? AND (p.data_postagem <= ? OR p.data_postagem IS NULL) ORDER BY p.titulo');
		$listarPaginacao->bindValue(1, 1);
		$listarPaginacao->bindValue(2, $linha);
        $listarPaginacao->bindValue(3, date('Y-m-d H:i:s'));
        ...
        if($total > 0){
            while($reg = $listar->fetch(PDO::FETCH_OBJ)){
                $receita['titulo'] = $reg->linha;
                $receita['corpo'] .=   '<div class="itemProduto">
            ...
            $receita['titulo'] = 'Linha não encontrada';
            $receita['corpo'] = "<p>Nenhum produto encontrado.</p>";
        }
        return $receita;
    }
    ...
    public function estruturaPaginacao($qtdPag, $pg, $prox, $ant){
        ...
        if ($qtdPag != 0) {
            ...
            if ($pg > 1) {
                $paginacao .= '<li><a href="produtos/linha/'.$linha.'/'.$ant.'">&#9664;</a></li>';
            } else {
                ...
                for ($i = 1; $i <= $qtdPag; $i++) {
                    if ($i == $pg) {
                        ...
                        if (($i == 1 ) || ($i == 2 ) || ($i == $pg - 2 ) || ($i == $pg - 1) || ($i == $pg + 1 ) || ($i == $pg + 2) || ($i == $qtdPag - 1) || ($i == $qtdPag)) {
                            $paginacao .= "<li><a " . ( ($pg == $i) ? 'class="ativo"' : '' ) . " href='produtos/linha/$linha/$i'>" . $i . "</a></li>";
                        }
                    }
                }
            }
            ...
            if ($pg < $qtdPag) {
                $paginacao .= '<li><a href="produtos/linha/'.$linha.'/'.$prox.'">&#9654;</a></li>';
            } else {
                $paginacao .= '<li class="desativado"><a href="#">&#9654;</a></li>';
            }
