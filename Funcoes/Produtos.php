...
class Produtos extends Conexao{
    ...
    public function listarTodosProdutos() {
        ...
        $listar = $pdo->prepare('SELECT p.*, l.linha as linha FROM `produtos` p INNER JOIN linhas l ON p.linha = l.id WHERE p.ativo= ? AND l.url =? AND (p.data_postagem <= ? OR p.data_postagem IS NULL) ORDER BY p.ordem, p.titulo LIMIT ' . $inicio . ", " . $limite);
	$listar->bindValue(1, 1);
	$listar->bindValue(2, $linha);
        ...
        $listarPaginacao = $pdo->prepare('SELECT p.*, l.linha as linha FROM `produtos` p INNER JOIN linhas l ON p.linha = l.id WHERE p.ativo= ? AND l.url =? AND (p.data_postagem <= ? OR p.data_postagem IS NULL)');
