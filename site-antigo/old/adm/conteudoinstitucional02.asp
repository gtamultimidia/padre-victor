...
<%
	...
	if request.querystring("atividade") = "editar" then
		...
		rs.open "select * FROM CONTEUDOINSTITUCIONAL where ID = " & ID, Conn, 1, 1
	...
	if request.querystring("atividade") = "excluir" then
		...
		ConectarBD Conn, "websitecafepadrevictor.mdb"
		Conn.execute "delete FROM CONTEUDOINSTITUCIONAL where ID = " & ID & " "
		FecharBD Conn
		session("msg") = "<img src=imagens/excluido.gif width=17 height=19 align=absmiddle>&nbsp;&nbsp; ITEM EXCLUÍDO COM SUCESSO."
		
		response.redirect "conteudoinstitucional01.asp"
	end if
	...
	if request.querystring("atividade") = "salvar" then
		...
		if testa_existencia = "sim" then
			Conn.execute "UPDATE CONTEUDOINSTITUCIONAL set titulo='" & titulo & "', resumo='" & resumo & "', materia='" & materia & "', datainsercao='" & datainsercao & "', dataexclusao='" & dataexclusao & "', thumbnail='" & thumbnail & "', nomenomenu='" & nomenomenu & "', albumfotos = " & albumfotos & " where ID=" & ID & " "			
			session("msg") = "<img src=imagens/ok_concluido.gif width=18 height=19 align=absmiddle>&nbsp;&nbsp;MENU " & UCASE(nomenomenu) & " - <b>""" & ucase(titulo) & """</b> ATUALIZADO(a) COM SUCESSO."
		else
			Conn.execute "insert INTO CONTEUDOINSTITUCIONAL (titulo, resumo, materia, datainsercao, dataexclusao, thumbnail, nomenomenu, albumfotos) values ('" & titulo & "','" & resumo & "','" & materia & "','" & datainsercao & "','" & dataexclusao & "','" & thumbnail & "','" & nomenomenu & "'," & albumfotos & ")"			
			session("msg") = "<img src=imagens/ok_concluido.gif width=18 height=19 align=absmiddle>&nbsp;&nbsp;MENU " & UCASE(nomenomenu) & " - <b>""" & ucase(titulo) & """</b> CRIADO(a) COM SUCESSO."
		end if
		FecharRecordset rs
		FecharBD Conn
		...
		if Campo_Form("arquivo") = "sim" then
			ConectarBD Conn, "websitecafepadrevictor.mdb"
			if ID = "0" then 
				set tb = Conn.execute("select top 1 ID FROM CONTEUDOINSTITUCIONAL order by ID desc")
			else
				set tb = Conn.execute("select top 1 ID FROM CONTEUDOINSTITUCIONAL where ID = " & ID)
			end if
			ID = tb("ID")
			set tb = nothing
			FecharBD Conn
			'response.redirect "conteudoinstitucional02.asp?ID=" & ID & "&atividade=editar"
			response.redirect "conteudoinstitucional01.asp"
		else
			'response.redirect "conteudoinstitucional02.asp?ID=" & ID & "&atividade=editar"
			response.redirect "conteudoinstitucional01.asp"
      ...
      <td valign="top"> 
        <div align="left"> 
          ...
          <table width="95%" align="center" border="0" cellspacing="0" cellpadding="10" class="table_color">
            <tr>
              <td><p><a href="acesso.asp">MENU</a> &#8250; <a href="conteudoinstitucional01.asp">CONTE&Uacute;DO INSTITUCIONAL </a> &#8250; CADASTRO </p></td>
            </tr>
          </table>
          <br>
          <table width="95%" align="center" border="0" cellspacing="0" cellpadding="10" class="table_color">
            <tr>
              <td><b><img src="imagens/logomarca_peq.gif" width="25" height="25" hspace="5" align="absmiddle"> CONTE&Uacute;DO INSTITUCIONAL: CADASTRO </b><br>
                ...
                <table border="0" cellpadding="0" cellspacing="0">
                  <form name="cadastro" method="post" action="conteudoinstitucional02.asp?atividade=salvar" onSubmit="return DFckForm(this, true)">
                    <tr>
                      ...
                      <td height="30"><table border="0" cellspacing="0" cellpadding="6">
                          <tr>
                            ...
                            <td><strong>Enviar imagens:</strong><br>
                              &#8250;&nbsp;<a href="upthumb.asp?largura=198&altura=119&largurapeq=60&alturapeq=45&campo=<%=Server.URLEncode("thumbnail")%>&pasta=<%=Server.URLEncode("upload/conteudoinstitucional")%>" onClick="abre_janela(500, 350, 'alterar_imagem')" target="alterar_imagem" class="texto_pagina">Imagens: tamanho m&aacute;ximo 5mb (Foto Horizontal) </a></td>
                          </tr>
                      </table></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <input name="nomenomenu" type="hidden" id="nomenomenu" value="Institucional" /></td>
                      <td height="30"><br>
