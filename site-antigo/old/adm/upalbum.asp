...
{
	document.forms['form'].action = "<%=Request.ServerVariables("SCRIPT_NAME")%>?largura=<%=largura%>&altura=<%=altura%>&pasta=<%= pasta %>&foto_album=<%= foto_album %>&enviar=sim&titulo= " + titulo;
...
<%
      ...
      MyFile.Close
		...
		Jpeg.New cint(LARGURA), cint(ALTURA), &HFFFFFF
	  ...
	  foto_descricao = Corrige_Campo(trim(request.querystring("titulo")))
	  foto_data = Cstr(date)
	  foto_arquivo = filename
	  foto_caminho = lcase(Corrige_Campo(request.querystring("pasta")) & "/" & filename)
	  foto_tamanho = LenB(value)
	  foto_album = Corrige_Campo(request.querystring("foto_album"))
	  Conn.execute "insert into FOTOS (foto_descricao, foto_data, foto_arquivo, foto_caminho, foto_tamanho, foto_album) values ('" & foto_descricao & "','" & foto_data & "','" & foto_arquivo & "','" & foto_caminho & "','" & foto_tamanho & "'," & foto_album & ")"
  ...
  <table width="100%" border="0" cellpadding="10" cellspacing="0" class="table_borda2">
    <tr>
      <td bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="3">
        <tr>
          <td><div align="right">Arquivo JPG:</div></td>
          <td>&nbsp;
