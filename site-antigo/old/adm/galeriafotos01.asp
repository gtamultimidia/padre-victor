...
<%
	...
	'-----------------------------------------------------------------------
		if (request.form <> "") and (request.querystring("busca")="sim") then
			...
			session("SQL") = "SELECT * from ALBUM where (1=1)"				
			if palavra_chave = " » BUSCA POR" then palavra_chave = ""
			if palavra_chave <> "" then session("SQL") = session("SQL") & " and ((alb_titulo like '%" & busca_inteligente(palavra_chave) & "%') or (alb_descricao like '%" & busca_inteligente(palavra_chave) & "%'))"
			session("SQL") = session("SQL") & " order by alb_data desc, alb_titulo"
		end if
		...
		if (sql = "") or ((request.querystring = "") and (request.form = "")) then
			SQL = "SELECT * from ALBUM order by alb_data desc, alb_titulo"
			session("SQL") = ""
		end if
...
<table width="95%" align="center" border="0" cellspacing="0" cellpadding="10" class="table_color">
  <tr>
    <td><a href="acesso.asp">MENU</a> &#8250; FOTOS</td>
  </tr>
</table>
<br>
          <table width="95%" border="0" align="center" cellpadding="10" cellspacing="0" class="table_color">
            <tr> 
              <td><img src="imagens/logomarca_peq.gif" width="25" height="25" hspace="5" align="absmiddle">&nbsp;<b>FOTOS: PESQUISA </b><br>
                <% if session("msg") <> "" then %>
                <br>
                ...
                <table width="95%" border="0" cellpadding="8" cellspacing="0" class="table_borda">
                  <form name="form1" method="post" action="galeriafotos01.asp">
                    <tr>
                      <td></td>
                      <td></td>
                      <td></td>
                ...
                <table width="95%" border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <td><table width="100%" border="0" cellpadding="5" cellspacing="0">
                      <tr>
                        <td>&nbsp;</td>
                        <td><div align="right">[&nbsp;<a href="galeriafotos02.asp?atividade=adicionar"  class="link-branco">NOVA GALERIA </a> ]</div></td>
                      </tr>
                    </table>
                      <table width="100%" border="0" cellpadding="4" cellspacing="1">
                        ...
                        <tr>
                          <td width="90%" height="25" background="imagens/tit_azulclaro_degrade.gif"><strong>NOME DO &Aacute;LBUM</strong></td>
                        ...
                        <tr>
                          <td bgcolor="<%= cor_fundo %>"><%= ucase(rs("alb_titulo")) %></td>
                          <td bgcolor="<%= cor_fundo %>"><div align="center"><%= formata_data(rs("alb_data")) %></div></td>
                          <td bgcolor="<%= cor_fundo %>"><div align="right"><a href="galeriafotos02.asp?alb_codigo=<%= rs("alb_codigo") %>&atividade=editar"><img src="imagens/bt_editar.gif" width="14" height="15" hspace="5" border="0" align="absmiddle" alt="EDITAR"></a><a href="galeriafotos02.asp?alb_codigo=<%= rs("alb_codigo") %>&atividade=excluir" onClick="return Apaga();"><img src="imagens/bt_excluir.gif" width="11" height="11" border="0" hspace="5" align="absmiddle" alt="EXCLUIR"></a></div></td>
                        </tr>
				  <%
						contador = contador + 1
						rs.movenext
